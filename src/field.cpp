...
void clear_boundaries(SoAField& field) {
    const std::size_t last_row = (field.ny - 1) * field.pitch;
    std::fill_n(field.real.begin(), field.pitch, 0.0f);
    std::fill_n(field.imag.begin(), field.pitch, 0.0f);
    std::fill_n(field.real.begin() + static_cast<std::ptrdiff_t>(last_row), field.pitch, 0.0f);
    std::fill_n(field.imag.begin() + static_cast<std::ptrdiff_t>(last_row), field.pitch, 0.0f);
