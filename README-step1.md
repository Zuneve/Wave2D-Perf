...
`src/solvers/euler_solver.cpp` - прямолинейная baseline-реализация без каких-либо
...
Для baseline-оценки это означает, что реализация уже находится в том диапазоне,
где дальнейший рост потребует не только `-O3`, но и изменения layout данных,
схемы доступа к памяти и более явной работы с SIMD.
...
4. **По этой грубой roofline-оценке baseline уже не выглядит совсем "пустым",**
   но остаётся неэффективным:
